...
export default async function DraftPage({ params }: { params: Promise<{ documentId: string }> }) {
  ...
  return <div><PageHeading title={document.draftReference} description="Draft only — no official number has been assigned and no capacity has been consumed."/><div className="mt-8"><DraftEditor documentId={document.id} isTestDocument={document.isTestDocument} customers={data.customers} items={data.items.map(i=>({...i,unitPrice:i.unitPrice.toString()}))} rates={data.rates.map(r=>({...r,value:r.value.toString()}))} trustedTax={trustedTax} initial={{type:document.type,customerId:document.customerId,customerName,currency:document.currency,draftDate:date(document.draftDate),dueDate:date(document.dueDate),notes:document.notes??"",lines:document.lines.map(line=>({id:line.id,catalogItemId:line.catalogItemId,customRateId:line.customRateId,description:line.description,quantity:line.quantity.toString(),unitPrice:line.unitPrice.toString()})),savedCalculation:document.taxCalculation as null | {base?:string;taxableValue?:string;taxTotal?:string;grossTotal?:string;components?:Array<{code:string;name:string;rate:string;amount:string}>}}}/></div>{issuePreview?<IssueDocumentPanel documentId={document.id} documentType={document.type} customerName={customerName || null} currency={document.currency} grandTotal={document.grandTotal.toFixed(2)} readiness={issuePreview[0].errors.map(({message})=>message)} capacity={issuePreview[1]}/>:null}<form action={archiveDraftAction.bind(null,document.id)} className="mt-8 rounded-xl border border-danger/40 p-5"><h2 className="font-bold text-text">Archive draft</h2><p className="mt-1 text-sm text-muted">This removes it from normal lists while preserving audit history.</p><button className="mt-4 min-h-11 rounded-lg border border-danger px-4 text-sm font-semibold text-danger">Archive Draft</button></form></div>;
}
