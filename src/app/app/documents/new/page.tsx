...
export default async function NewDocumentPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  const trustedTax = data.trustedTaxVersion
    ...
    : null;
  const today=new Date().toISOString().slice(0,10); return <div><PageHeading title="Create Draft" description="Build a structured business document. Saving a draft does not issue it or consume document capacity."/><div className="mt-8"><DraftEditor documentId={null} customers={data.customers} items={data.items.map(i=>({...i,unitPrice:i.unitPrice.toString()}))} rates={data.rates.map(r=>({...r,value:r.value.toString()}))} trustedTax={trustedTax} initial={{type,customerId:null,currency:data.context.workspace.currency,draftDate:today,dueDate:"",notes:"",lines:[{catalogItemId:null,customRateId:null,description:"",quantity:"1",unitPrice:"0.00"}],savedCalculation:null}}/></div></div>;
}
