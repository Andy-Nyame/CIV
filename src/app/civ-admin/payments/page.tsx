...
export default async function PlatformPaymentsPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-6 rounded-xl border border-civ-blue bg-active p-4 text-sm leading-6 text-text">Paystack {label(data.mode)} Mode · Document Credits are one-time payments; recurring subscriptions are card-only. Test totals are operational data, never production revenue. Refund and live-payment controls remain disabled.</p>
      ...
      <section className="mt-7 overflow-hidden rounded-xl border border-border bg-surface" aria-labelledby="platform-payments-title">
        ...
        {data.payments.length ? (
          <div className="overflow-x-auto"><table className="w-full min-w-[76rem] text-left text-sm"><thead className="bg-surface-muted text-muted"><tr><th className="px-4 py-3 font-semibold">Reference</th><th className="px-4 py-3 font-semibold">Workspace</th><th className="px-4 py-3 font-semibold">Initiated by</th><th className="px-4 py-3 font-semibold">Purpose</th><th className="px-4 py-3 font-semibold">Amount</th><th className="px-4 py-3 font-semibold">Status</th><th className="px-4 py-3 font-semibold">Fulfillment</th><th className="px-4 py-3 font-semibold">Attempts</th><th className="px-4 py-3 font-semibold">Created</th></tr></thead><tbody className="divide-y divide-border">{data.payments.map((payment) => <tr key={payment.id}><td className="px-4 py-4 font-mono text-xs text-text">{payment.internalReference}</td><td className="px-4 py-4 text-text">{payment.workspace.name}</td><td className="px-4 py-4"><p className="text-text">{payment.initiatedBy?.name || "CIV system"}</p><p className="mt-0.5 text-xs text-muted">{payment.initiatedBy?.email ?? "Recurring billing"}</p></td><td className="px-4 py-4 text-text">{label(payment.purpose)}</td><td className="px-4 py-4 text-text">{payment.currency === "GHS" ? "GH₵" : `${payment.currency} `}{payment.amount}</td><td className="px-4 py-4 text-text">{label(payment.status)}</td><td className="px-4 py-4 text-text">{payment.documentCreditPurchase ? `${label(payment.documentCreditPurchase.status)} · ${payment.documentCreditPurchase.pack.code}` : payment.subscriptionChange ? `${label(payment.subscriptionChange.status)} · ${payment.subscriptionChange.targetPlanCodeSnapshot}` : payment.subscriptionBillingPeriod ? `${label(payment.subscriptionBillingPeriod.status)} renewal` : "Infrastructure only"}</td><td className="px-4 py-4 text-text">{payment.attempts}</td><td className="px-4 py-4 text-muted"><LocalDateTime value={payment.createdAt.toISOString()} /></td></tr>)}</tbody></table></div>
