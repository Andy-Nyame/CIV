...
export function CustomerForm({ customer }: { customer?: { id: string; name: string; email: string | null; phone: string | null; address: string | null; businessTin: string | null; taxpayerIdType: "GHANA_CARD_PIN" | "GRA_TIN" | null; taxpayerId: string | null; vatRegistrationStatus: "NOT_REGISTERED" | "PENDING" | "REGISTERED" | "DEREGISTERED" | null; taxStatus: "ORDINARY_CONSUMER" | "TAXABLE_PERSON"; notes: string | null } }) {
  ...
  return <form action={action} className="grid gap-4">
    ...
    <label className="grid gap-1.5 text-sm font-semibold text-text">Customer tax profile<select className={field} name="taxStatus" defaultValue={customer?.taxStatus ?? "ORDINARY_CONSUMER"}><option value="ORDINARY_CONSUMER">Ordinary / non-taxable customer</option><option value="TAXABLE_PERSON">Taxable / business customer</option></select><span className="font-normal text-muted">Taxable customers require their address and taxpayer identity before a tax document can be issued.</span></label>
