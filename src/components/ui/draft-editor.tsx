...
type PaymentEvent = { occurredAt: string; amount: string; method: string | null; reference: string | null };
...
export function DraftEditor({ documentId, initial, customers, items, rates, trustedTax, workspaceVat, originalDocuments, isTestDocument = false }: {
  ...
  initial: {
    ...
    priceMode: "TAX_EXCLUSIVE" | "TAX_INCLUSIVE"; receiptType: "COMMERCIAL" | "VAT_SALES_RECEIPT";
    servicePeriodStart: string; servicePeriodEnd: string; originalDocumentId: string | null; adjustmentReason: string;
    customerAddress: string; customerTaxpayerIdType: "GHANA_CARD_PIN" | "GRA_TIN" | ""; customerTaxpayerId: string;
    customerVatRegistrationStatus: "NOT_REGISTERED" | "PENDING" | "REGISTERED" | "DEREGISTERED" | ""; customerTaxStatus: "ORDINARY_CONSUMER" | "TAXABLE_PERSON";
    withholdingApplied: boolean; withholdingAgent: boolean; withholdingAmount: string; withholdingReference: string; withholdingEvidence: string; withholdingDate: string;
  ...
  workspaceVat: { environment: "NORMAL" | "TEST"; vatRegistrationStatus: "NOT_REGISTERED" | "PENDING" | "REGISTERED" | "DEREGISTERED"; vatRegistrationEffectiveDate: Date | string | null; vatDeregistrationEffectiveDate: Date | string | null; vatSalesReceiptAuthorization: "UNKNOWN" | "NOT_AUTHORIZED" | "AUTHORIZED" };
  ...
  const [receiptType, setReceiptType] = useState(initial.receiptType);
  const [transactionType, setTransactionType] = useState(initial.transactionType);
  const [customerTaxStatus, setCustomerTaxStatus] = useState(initial.customerTaxStatus);
  const [customerAddress, setCustomerAddress] = useState(initial.customerAddress);
  const [customerTaxpayerIdType, setCustomerTaxpayerIdType] = useState(initial.customerTaxpayerIdType);
  const [customerTaxpayerId, setCustomerTaxpayerId] = useState(initial.customerTaxpayerId);
  const [customerVatStatus, setCustomerVatStatus] = useState(initial.customerVatRegistrationStatus);
  const [paymentEvents, setPaymentEvents] = useState(initial.paymentEvents);
  const [withholdingApplied, setWithholdingApplied] = useState(initial.withholdingApplied);
  const statutoryPath = documentType === "VAT_INVOICE" || (documentType === "RECEIPT" && receiptType === "VAT_SALES_RECEIPT");
  const isAdjustment = documentType === "CREDIT_NOTE" || documentType === "DEBIT_NOTE";
  const supplierVatEligible = useMemo(() => {
    const taxPoint = [supplyDate, draftDate, ...paymentEvents.map(({ occurredAt }) => occurredAt)]
      .filter(Boolean)
      .sort()[0] ?? draftDate;
    return statutoryPath && isVatEligibleAtTaxPoint(workspaceVat, taxPoint);
  }, [draftDate, paymentEvents, statutoryPath, supplyDate, workspaceVat]);
  ...
  const updateCustomerName = (name: string) => {
    ...
    if (saved) {
      setCustomerTaxStatus(saved.taxStatus); setCustomerAddress(saved.address ?? "");
      setCustomerTaxpayerIdType(saved.taxpayerIdType ?? ""); setCustomerTaxpayerId(saved.taxpayerId ?? "");
      setCustomerVatStatus(saved.vatRegistrationStatus ?? "");
    } else {
      setCustomerTaxStatus("ORDINARY_CONSUMER"); setCustomerAddress(""); setCustomerTaxpayerIdType(""); setCustomerTaxpayerId(""); setCustomerVatStatus("");
    }
  };
  ...
  return <form action={action} className="grid gap-7">
    ...
    <input type="hidden" name="lines" value={JSON.stringify(lines)}/><input type="hidden" name="paymentEvents" value={JSON.stringify(paymentEvents)}/><input type="hidden" name="customerId" value={customerId ?? ""}/>
    <section className="grid gap-4 rounded-xl border border-border bg-surface p-5 sm:grid-cols-2 sm:p-6">
      ...
      <label className="grid gap-1.5 text-sm font-semibold text-text">Transaction type<select className={field} name="transactionType" value={transactionType} onChange={(event) => setTransactionType(event.target.value as typeof transactionType)}><option value="SALE">Sale</option><option value="SERVICE">Service</option><option value="HIRE_OR_LEASE">Hire or lease</option><option value="EXCHANGE">Exchange</option><option value="OTHER">Other</option></select></label>
      {documentType === "RECEIPT" ? <label className="grid gap-1.5 text-sm font-semibold text-text sm:col-span-2">Receipt classification<select className={field} name="receiptType" value={receiptType} onChange={(event) => setReceiptType(event.target.value as typeof receiptType)}><option value="COMMERCIAL">Commercial receipt</option><option value="VAT_SALES_RECEIPT" disabled={workspaceVat.environment === "NORMAL" && workspaceVat.vatSalesReceiptAuthorization !== "AUTHORIZED"}>VAT sales receipt{workspaceVat.environment === "NORMAL" && workspaceVat.vatSalesReceiptAuthorization !== "AUTHORIZED" ? " (authorization not recorded)" : ""}</option></select></label> : <input type="hidden" name="receiptType" value="COMMERCIAL"/>}
      ...
      <label className="grid gap-1.5 text-sm font-semibold text-text">Supply date and time<input className={field} type="datetime-local" name="supplyDate" value={supplyDate} onChange={(event)=>setSupplyDate(event.target.value)} required/></label>
      ...
      {transactionType === "HIRE_OR_LEASE" ? <><label className="grid gap-1.5 text-sm font-semibold text-text">Service / rental period start<input className={field} type="datetime-local" name="servicePeriodStart" defaultValue={initial.servicePeriodStart} required/></label><label className="grid gap-1.5 text-sm font-semibold text-text">Service / rental period end<input className={field} type="datetime-local" name="servicePeriodEnd" defaultValue={initial.servicePeriodEnd} required/></label></> : <><input type="hidden" name="servicePeriodStart" value=""/><input type="hidden" name="servicePeriodEnd" value=""/></>}
    ...
    <section className="rounded-xl border border-border bg-surface p-5 sm:p-6"><label className="grid gap-1.5 text-sm font-semibold text-text">Customer name *<input className={field} name="customerName" value={customerName} onChange={(event)=>updateCustomerName(event.target.value)} list="saved-customer-names" maxLength={200} required autoComplete="off"/></label><datalist id="saved-customer-names">{customers.map((saved)=><option value={saved.name} key={saved.id}/>)}</datalist><p className="mt-2 text-sm text-muted">Start typing to reuse a saved customer, or enter a new name.</p><div className="mt-4 border-t border-border pt-4"><label className="grid gap-1.5 text-sm font-semibold text-text">Customer tax classification<select className={field} name="customerTaxStatus" value={customerTaxStatus} onChange={(event)=>setCustomerTaxStatus(event.target.value as typeof customerTaxStatus)}><option value="ORDINARY_CONSUMER">Ordinary / non-taxable customer</option><option value="TAXABLE_PERSON">Taxable / business customer</option></select></label>{customerTaxStatus === "TAXABLE_PERSON" ? <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-sm font-semibold text-text sm:col-span-2">Business address<input className={field} name="customerAddress" value={customerAddress} onChange={(event)=>setCustomerAddress(event.target.value)} required maxLength={2000}/></label><label className="grid gap-1.5 text-sm font-semibold text-text">Taxpayer ID type<select className={field} name="customerTaxpayerIdType" value={customerTaxpayerIdType} onChange={(event)=>setCustomerTaxpayerIdType(event.target.value as typeof customerTaxpayerIdType)} required><option value="">Select type</option><option value="GHANA_CARD_PIN">Ghana Card PIN</option><option value="GRA_TIN">GRA TIN</option></select></label><label className="grid gap-1.5 text-sm font-semibold text-text">Taxpayer ID<input className={field} name="customerTaxpayerId" value={customerTaxpayerId} onChange={(event)=>setCustomerTaxpayerId(event.target.value)} required maxLength={100}/></label><label className="grid gap-1.5 text-sm font-semibold text-text sm:col-span-2">Recorded VAT status<select className={field} name="customerVatRegistrationStatus" value={customerVatStatus} onChange={(event)=>setCustomerVatStatus(event.target.value as typeof customerVatStatus)}><option value="">Not recorded</option><option value="NOT_REGISTERED">Not registered</option><option value="PENDING">Pending</option><option value="REGISTERED">Registered</option><option value="DEREGISTERED">Deregistered</option></select></label></div> : <><input type="hidden" name="customerAddress" value=""/><input type="hidden" name="customerTaxpayerIdType" value=""/><input type="hidden" name="customerTaxpayerId" value=""/><input type="hidden" name="customerVatRegistrationStatus" value=""/></>}</div></section>
    <section className="border-l-4 border-civ-blue bg-soft-blue p-5 dark:bg-surface-muted"><h2 className="font-bold text-text">Ghana tax treatment</h2><p className="mt-1 text-sm text-muted">{supplierVatEligible ? `VAT eligible at the current tax point${trustedTax ? ` · ${trustedTax.name} ${trustedTax.version}` : ""}.` : statutoryPath ? "No VAT-family tax will be charged at the current tax point." : "This is a commercial-document path; statutory VAT-family taxes are not applied."} Each line carries its own treatment; custom rates remain separate.</p>{statutoryPath ? <p className="mt-2 text-sm font-semibold text-danger">CIV helps prepare Ghana VAT-compliant data. Official VAT tax documents require applicable GRA Certified Invoicing System processing.</p> : null}{trustedTax?<ul className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-text">{trustedTax.components.map((component)=><li key={component.code}><strong>{component.code}</strong> {Number(component.rate)}%</li>)}</ul>:null}</section>
    <section aria-labelledby="line-items-title"><div className="flex items-center justify-between gap-3"><div><h2 id="line-items-title" className="text-xl font-bold text-text">Line items</h2><p className="mt-1 text-sm text-muted">Saved totals are recalculated with exact decimal arithmetic on the server.</p></div><button type="button" className="min-h-11 rounded-lg border border-civ-blue px-4 text-sm font-semibold text-link" onClick={()=>setLines(current=>[...current,blankLine()])}>Add line</button></div>
      <div className="mt-4 grid gap-4">{lines.map((line,index)=><article key={line.id ?? index} className="rounded-xl border border-border bg-surface p-4 sm:p-5"><div className="flex items-center justify-between"><h3 className="font-bold text-text">Line {index+1}</h3><button type="button" disabled={lines.length===1} className="min-h-11 px-3 text-sm font-semibold text-danger disabled:opacity-40" onClick={()=>setLines(current=>current.filter((_,i)=>i!==index))}>Remove</button></div><div className="mt-3 grid gap-4 sm:grid-cols-2">
        ...
        {line.taxTreatment!=="STANDARD_RATED"?<label className="grid gap-1.5 text-sm font-semibold text-text sm:col-span-2">Classification/support reference<input className={field} value={line.taxTreatmentReference??""} onChange={e=>update(index,{taxTreatmentReference:e.target.value})} required/></label>:null}
    ...
    <section className="rounded-xl border border-border bg-surface p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold text-text">Payment / tax-point events</h2><p className="mt-1 text-sm text-muted">Record only real payments. CIV uses the earliest real supply, payment, or document event available.</p></div><button type="button" className="min-h-11 rounded-lg border border-civ-blue px-4 text-sm font-semibold text-link" onClick={()=>setPaymentEvents((current)=>[...current,{occurredAt:supplyDate,amount:"0.00",method:null,reference:null}])}>Add payment</button></div><div className="mt-4 grid gap-3">{paymentEvents.map((payment,index)=><div className="grid gap-3 rounded-lg border border-border p-3 sm:grid-cols-5" key={index}><label className="grid gap-1 text-sm font-semibold text-text sm:col-span-2">Payment date/time<input className={field} type="datetime-local" value={payment.occurredAt} onChange={(event)=>setPaymentEvents((current)=>current.map((item,i)=>i===index?{...item,occurredAt:event.target.value}:item))} required/></label><label className="grid gap-1 text-sm font-semibold text-text">Amount<input className={field} inputMode="decimal" value={payment.amount} onChange={(event)=>setPaymentEvents((current)=>current.map((item,i)=>i===index?{...item,amount:event.target.value}:item))} required/></label><label className="grid gap-1 text-sm font-semibold text-text">Method<input className={field} value={payment.method??""} onChange={(event)=>setPaymentEvents((current)=>current.map((item,i)=>i===index?{...item,method:event.target.value||null}:item))}/></label><button type="button" className="min-h-11 self-end text-sm font-semibold text-danger" onClick={()=>setPaymentEvents((current)=>current.filter((_,i)=>i!==index))}>Remove</button><label className="grid gap-1 text-sm font-semibold text-text sm:col-span-5">Reference<input className={field} value={payment.reference??""} onChange={(event)=>setPaymentEvents((current)=>current.map((item,i)=>i===index?{...item,reference:event.target.value||null}:item))}/></label></div>)}</div></section>
    <section className="rounded-xl border border-border bg-surface p-5"><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-text"><input type="checkbox" name="withholdingApplied" checked={withholdingApplied} onChange={(event)=>setWithholdingApplied(event.target.checked)}/>Record certified VAT withholding</label>{withholdingApplied?<div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="flex min-h-11 items-center gap-2 text-sm font-semibold text-text sm:col-span-2"><input type="checkbox" name="withholdingAgent" defaultChecked={initial.withholdingAgent} required/>Recipient is an appointed VAT withholding agent</label><label className="grid gap-1.5 text-sm font-semibold text-text">Withheld amount<input className={field} name="withholdingAmount" defaultValue={initial.withholdingAmount} inputMode="decimal" required/></label><label className="grid gap-1.5 text-sm font-semibold text-text">Certificate/reference<input className={field} name="withholdingReference" defaultValue={initial.withholdingReference} required/></label><label className="grid gap-1.5 text-sm font-semibold text-text">Certificate date<input className={field} type="date" name="withholdingDate" defaultValue={initial.withholdingDate}/></label><label className="grid gap-1.5 text-sm font-semibold text-text">Evidence reference<input className={field} name="withholdingEvidence" defaultValue={initial.withholdingEvidence}/></label></div>:<><input type="hidden" name="withholdingAgent" value="false"/><input type="hidden" name="withholdingAmount" value="0"/><input type="hidden" name="withholdingReference" value=""/><input type="hidden" name="withholdingEvidence" value=""/><input type="hidden" name="withholdingDate" value=""/></>}<p className="mt-2 text-sm text-muted">Withholding is a settlement credit and never reduces statutory output tax.</p></section>
