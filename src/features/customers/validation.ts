...
export const customerInputSchema = z.object({
  ...
  taxStatus: z.enum(["ORDINARY_CONSUMER", "TAXABLE_PERSON"]).default("ORDINARY_CONSUMER"),
  ...
  if (value.taxStatus === "TAXABLE_PERSON" && (!value.address || !value.taxpayerId || !value.taxpayerIdType)) {
    context.addIssue({ code: "custom", path: ["taxStatus"], message: "A taxable/business customer needs an address and taxpayer identity." });
  }
});
