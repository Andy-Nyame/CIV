...
export const issuedDocumentSnapshotSchema = z.object({
  ...
  document: z.object({
    ...
    adjustment: z.object({
      ...
      originalDocumentNumber: z.string().min(1).max(100),
  ...
  issuer: z.object({
    ...
    businessActivity: z.enum(["GOODS", "SERVICES", "BOTH"]).nullable().optional().default(null),
    vatRegistrationStatus: z.enum(["NOT_REGISTERED", "PENDING", "REGISTERED", "DEREGISTERED"]).nullable().optional().default(null),
    vatRegistrationEffectiveDate: z.string().date().nullable().optional().default(null),
    vatDeregistrationEffectiveDate: z.string().date().nullable().optional().default(null),
  ...
  customer: z.object({
    id: z.string().uuid().nullable(), name: z.string().min(1).max(200), email: nullableText, phone: nullableText, address: nullableText, businessTin: nullableText,
    taxpayerIdType: z.enum(["GHANA_CARD_PIN", "GRA_TIN"]).nullable().optional().default(null),
    taxpayerId: nullableText.optional().default(null),
    vatRegistrationStatus: z.enum(["NOT_REGISTERED", "PENDING", "REGISTERED", "DEREGISTERED"]).nullable().optional().default(null),
  }).strict().nullable(),
  lines: z.array(z.object({
    ...
    unitOfMeasure: nullableText.optional().default(null),
    originalAmount: moneySchema.optional().default("0.00"),
    discount: moneySchema.optional().default("0.00"),
    taxTreatment: z.enum(["STANDARD_RATED", "ZERO_RATED", "EXEMPT"]).optional().default("STANDARD_RATED"),
    taxTreatmentReason: nullableText.optional().default(null),
    taxTreatmentReference: nullableText.optional().default(null),
    relief: z.object({ reason: z.string().min(1).max(500), reference: z.string().min(1).max(500) }).strict().nullable().optional().default(null),
    taxableBase: moneySchema.optional().default("0.00"),
    tax: z.object({ amount: moneySchema, components: z.array(taxComponentSnapshotSchema).max(20) }).strict().nullable().optional().default(null),
  ...
  totals: z.object({
    subtotal: moneySchema, discount: moneySchema, customRates: moneySchema, taxableValue: moneySchema, trustedTax: moneySchema, grandTotal: moneySchema,
    originalAmount: moneySchema.optional().default("0.00"),
    standardRatedValue: moneySchema.optional().default("0.00"),
    zeroRatedValue: moneySchema.optional().default("0.00"),
...
export function buildIssuerSnapshot(workspace: {
  ...
  businessActivity?: "GOODS" | "SERVICES" | "BOTH" | null;
  vatRegistrationStatus?: "NOT_REGISTERED" | "PENDING" | "REGISTERED" | "DEREGISTERED" | null;
  vatRegistrationEffectiveDate?: Date | null; vatDeregistrationEffectiveDate?: Date | null;
  ...
  return {
    ...
    businessActivity: workspace.businessActivity ?? null,
    vatRegistrationStatus: workspace.vatRegistrationStatus ?? null,
    vatRegistrationEffectiveDate: workspace.vatRegistrationEffectiveDate?.toISOString().slice(0, 10) ?? null,
    vatDeregistrationEffectiveDate: workspace.vatDeregistrationEffectiveDate?.toISOString().slice(0, 10) ?? null,
...
export function buildLineSnapshots(lines: Array<{
  ...
  unitOfMeasure?: string | null; originalAmount?: Prisma.Decimal; discountAmount?: Prisma.Decimal;
  taxTreatment?: "STANDARD_RATED" | "ZERO_RATED" | "EXEMPT"; taxTreatmentReason?: string | null;
  taxTreatmentReference?: string | null; reliefApplied?: boolean; reliefReason?: string | null; reliefReference?: string | null;
  taxableBase?: Prisma.Decimal; taxTotal?: Prisma.Decimal; taxCalculation?: Prisma.JsonValue | null;
}>) {
  return lines.map((line) => {
    const parsedComponents = z.array(taxComponentSnapshotSchema).safeParse(
      (line.taxCalculation as { components?: unknown } | null)?.components ?? [],
    );
    return {
    ...
    originalAmount: (line.originalAmount ?? line.lineSubtotal).toFixed(2),
    discount: (line.discountAmount ?? new Prisma.Decimal(0)).toFixed(2),
    taxTreatment: line.taxTreatment ?? "STANDARD_RATED",
    taxTreatmentReason: line.taxTreatmentReason ?? null,
    taxTreatmentReference: line.taxTreatmentReference ?? null,
    relief: line.reliefApplied && line.reliefReason && line.reliefReference ? { reason: line.reliefReason, reference: line.reliefReference } : null,
    taxableBase: (line.taxableBase ?? line.lineSubtotal).toFixed(2),
    tax: line.taxTotal?.gt(0) ? {
      amount: line.taxTotal.toFixed(2),
...
export function buildIssuedDocumentSnapshot(input: {
  document: {
    id: string; draftReference: string; type: "INVOICE" | "RECEIPT" | "VAT_INVOICE" | "CREDIT_NOTE" | "DEBIT_NOTE"; currency: string;
  ...
  const payload = {
    ...
    totals: {
      ...
      originalAmount: input.document.lines.reduce((sum, line) => sum.add(line.originalAmount ?? line.lineSubtotal), new Prisma.Decimal(0)).toFixed(2),
      standardRatedValue: (input.document.standardRatedValue ?? new Prisma.Decimal(0)).toFixed(2),
      zeroRatedValue: (input.document.zeroRatedValue ?? new Prisma.Decimal(0)).toFixed(2),
      exemptValue: (input.document.exemptValue ?? new Prisma.Decimal(0)).toFixed(2),
      relievedValue: (input.document.relievedValue ?? new Prisma.Decimal(0)).toFixed(2),
      totalTaxInclusiveValue: input.document.subtotal.add(input.document.taxTotal).toFixed(2),
      withholding: input.document.withholdingApplied && input.document.withholdingAmount?.gt(0) && input.document.withholdingReference ? {
  ...
  return db.$transaction(async (transaction) => {
    ...
    return {
      ...
      totals: {
        ...
        withholdingAmount: document.withholdingAmount.toFixed(2),
